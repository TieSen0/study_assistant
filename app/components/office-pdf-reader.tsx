  ...
  return (
    <main className="office-reader-shell">
      ...
      <nav className="office-ribbon" aria-label="阅读工具">
        <div className="office-tabs"><button type="button" className="active">阅读</button><button type="button" onClick={() => setMarking((value) => !value)} className={marking ? "active" : ""}>批注</button><span className="office-find-control"><button type="button" onClick={() => setFindOpen((value) => !value)} className={findOpen ? "active" : ""}>查找</button>{findOpen && <span className="office-find-popover"><Search size={15} /><input value={findText} onChange={(event) => setFindText(event.target.value)} placeholder="查找" autoFocus /><button type="button" onClick={() => setFindOpen(false)} aria-label="关闭查找"><X size={14} /></button><details><summary>选项</summary><label><input type="checkbox" checked={wholeWord} onChange={(event) => setWholeWord(event.target.checked)} /> 整词</label><label><input type="checkbox" checked={matchCase} onChange={(event) => setMatchCase(event.target.checked)} /> 区分大小写</label><label>范围<select value={findScope} onChange={(event) => setFindScope(event.target.value as "document" | "all")}><option value="document">原文</option><option value="all">全部</option></select></label></details>{findText.trim() && <span className="office-search-results">{searchResults.length ? searchResults.slice(0, 8).map((result, index) => <button key={`${result.source}-${result.pageNumber}-${index}`} type="button" onClick={() => { changePage(result.pageNumber); setFindOpen(false); }}><b>{result.source} · 第 {result.pageNumber} 页</b><span>{result.excerpt}</span></button>) : <p>未找到匹配项</p>}<small>{searchResults.length > 8 ? `显示前 8 条，共 ${searchResults.length} 条` : `${searchResults.length} 条结果`}</small></span>}</span>}</span><button type="button" onClick={() => alert("翻译服务尚未配置。后续会在这里显示划词和选区翻译。")}>翻译</button></div>
