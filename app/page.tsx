...
export default function Home() {
  ...
  function saveAgentNote(event: FormEvent) {
    event.preventDefault();
    if (!activeDocument || !agentDraft.trim()) return;
    setAgentNotes((items) => [...items, agentDraft.trim()]);
    setAgentDraft("");
  }
  ...
  return <main className="vscode-shell">
    ...
    <section className="vscode-workbench">
      ...
      {agentOpen ? <AgentSidebar document={activeDocument} draft={agentDraft} notes={agentNotes} onDraft={setAgentDraft} onSave={saveAgentNote} onClose={() => setAgentOpen(false)} /> : <button type="button" className="vscode-reopen-secondary" onClick={() => setAgentOpen(true)}><Bot size={18} /> Agent</button>}
    </section>
...
function NotesSidebar({ notes }: { notes: string[] }) { return <><header className="vscode-sidebar-title"><strong>NOTES</strong></header><section className="vscode-search-results">{notes.length ? notes.map((note, index) => <article key={`${note}-${index}`}><b>本地问题</b><p>{note}</p></article>) : <p>在右侧 Agent 面板保存的问题会出现在这里。</p>}</section></>; }
...
function AgentSidebar({ document, draft, notes, onDraft, onSave, onClose }: { document: StoredDocument | null; draft: string; notes: string[]; onDraft: (value: string) => void; onSave: (event: FormEvent) => void; onClose: () => void }) { return <aside className="vscode-secondary"><header className="vscode-sidebar-title"><strong>LENS AGENT</strong><button type="button" onClick={onClose} aria-label="关闭次级侧栏"><PanelRight size={16} /></button></header><div className="vscode-agent-context"><Bot size={19} /><div><b>{document ? document.title : "未选择文件"}</b><small>{document ? `文档级上下文 · ${document.page_count} 页` : "打开一份资料后即可提问"}</small></div></div><section className="vscode-agent-thread">{notes.length ? notes.map((note, index) => <article key={`${note}-${index}`}><p>{note}</p><small>本地问题 · 尚未调用模型</small></article>) : <p>此处是 VSCode 的 Secondary Side Bar。模型接入前，只保留你针对当前文档提出的问题。</p>}</section><form className="vscode-agent-compose" onSubmit={onSave}><textarea value={draft} onChange={(event) => onDraft(event.target.value)} disabled={!document} placeholder={document ? "针对当前文档提问…" : "先从资源管理器打开一份资料"} rows={3} /><button type="submit" disabled={!document || !draft.trim()}>保存问题 <Send size={13} /></button><small>不会发送给外部模型</small></form></aside>; }
