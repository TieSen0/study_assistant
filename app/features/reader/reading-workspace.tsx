...
import { Bookmark, ChevronLeft, ChevronRight, ListTree, LoaderCircle, PanelRightOpen, StickyNote, X } from "lucide-react";
import { ReaderAgentPanel } from "./agent-panel";
...
export function OfficePdfReader(props: ReaderProps) {
  const { documentId, title, pageCount, initialPage, pages, marks, onExit, onPageChange, onCreateMark, onAsk, onEditMark, onDeleteMark, onSetDoubtStatus } = props;
  ...
  return <main className="office-reader-shell">
    <ReaderToolbar title={title} marking={marking} zoom={zoom} view={view} findOpen={findOpen} findText={findText} results={searchResults} onExit={onExit} onToggleMarking={() => setMarking((value) => !value)} onZoom={(delta) => setZoom((value) => clamp(value + delta, 0.5, 2.4))} onFit={fitPage} onViewChange={setView} onToggleFind={() => setFindOpen((value) => !value)} onFindChange={setFindText} onCloseFind={() => setFindOpen(false)} onJump={jumpToPage} />
    <div className="office-reader-body">
      {sidebarOpen && <ReaderSidebar navigation={navigation} onNavigationChange={setNavigation} onLibrary={onExit} onClose={() => setSidebarOpen(false)}>
      ...
      {!sidebarOpen && <button type="button" className="reader-reopen-panel left" onClick={() => setSidebarOpen(true)} title="显示功能总览">资料</button>}
      <section className={`office-document-stage ${view}`} ref={viewportRef}>
        ...
        {selection && <div className="office-selection-actions"><span>已选择第 {selection.pageNumber} 页区域</span><button type="button" onClick={() => { onCreateMark("note", selection.pageNumber, selection.anchor); setSelection(null); }}>批注</button><button type="button" onClick={() => setAgentOpen(true)}>询问 Agent</button><button type="button" onClick={() => { onCreateMark("doubt", selection.pageNumber, selection.anchor); setSelection(null); }}>存疑</button><button type="button" onClick={() => { onCreateMark("bookmark", selection.pageNumber, selection.anchor); setSelection(null); }}>书签</button><button type="button" onClick={() => setSelection(null)} aria-label="取消选择"><X size={14} /></button></div>}
      </section>
      {agentOpen ? <ReaderAgentPanel selection={selection} onAsk={onAsk} onCollapse={() => setAgentOpen(false)} /> : <button type="button" className="reader-reopen-panel right" onClick={() => setAgentOpen(true)} title="打开阅读 Agent"><PanelRightOpen size={18} /></button>}
    </div>
